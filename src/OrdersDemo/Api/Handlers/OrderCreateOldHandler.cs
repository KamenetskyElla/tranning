...
public record OrderCreateOldRequest(OrderCreateDto CreateDto) : IRequest<OrderDto>;
public class OrderCreateOldHandler(OrderDbContext dbContext,
                              IMapper mapper,
                              IOrderCalculator orderCalculator,
                              IDateTime dateTime,
                              IDocumentNoGenerator documentNoGenerator) : IRequestHandler<OrderCreateOldRequest, OrderDto>
{



    public async Task<OrderDto> Handle(OrderCreateOldRequest request, CancellationToken cancellationToken)
    {
